...
def cor_fast(x, y=None, method="pearson", ncores=4):
    ...
    if method == "kendall":
        if y is None:
            ...
            results = [pool.apply_async(cal_kendall, args=(
...
def cor_convert(rho, case):
    ...
    if case not in switch:
        raise ValueError(
            "Invalid conversion case. Expected one of: %s" % list(switch))
...
def cor_bounds(margins, type="pearson", cores=1, reps=1e5):
    ...
    for (i, j) in index_mat:
        rho_lower[i, j] = cor_fast(
            sim_data[:, i], np.flip(sim_data[:, j]), method=type)
